...
function selectPlayer(position){
    ...
    // const options = players.find(player => player.position === position);
    ...
    options.forEach(player => {
        ...
        option.className = "flex items-center w-full border border-black bg-slate-800 object-cover";
        // option.classList.add("w-12");
        ...
        // option.innerHTML = `<img src="${player.photo}" width="50" height="50"> ${player.name}`;
        // option.textContent = player.name; 
        option.appendChild(img);
        option.appendChild(name);

        selectElement.appendChild(option);
        selectElement.innerHTML = selectElement.innerHTML + option.outerHTML;
        // alert(player.photo);
